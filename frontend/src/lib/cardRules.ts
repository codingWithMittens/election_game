...
export function getSelectionDescription(card: Card): string {
  ...
  if (rules.type === 'AUTO') {
    ...
    return `Auto-applies to: ${rules.validStates.join(', ')}`;
  }
...
/**
 * Check if a state is valid for selection given the card and current selections
 */
export function isStateSelectable(
  ...
  // AUTO cards don't allow manual selection
  if (rules.type === 'AUTO') {
    return false;
  }

  // If state is already selected, it's "selectable" (for deselection)
  if (currentlySelected.includes(stateAbbr)) {
    return true;
  }

  // Check if we've hit the max selection limit
  if (currentlySelected.length >= rules.maxStates) {
    return false;
  }

  // Check if state is in valid states list (if restricted)
  if (rules.validStates.length > 0) {
    return rules.validStates.includes(stateAbbr);
  }
